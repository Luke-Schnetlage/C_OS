...
void mq_send(struct msg_queue *mq, struct mq_msg *message) {
  ...
  //efficent wait
  if ((&mq->in - &mq->out) >=
      QUEUE_SIZE) { // While the queue is full, wait on the queue’s msg_removed condition variable
    pthread_cond_wait(&mq->msg_removed,&mq->lock); 
  }

  // Copy the message from the mq_msg structure provided to the correct locationin the queue.
