...
struct thread_args {
  ...
  struct msg_queue *mq; // pointer to the queue that the thread will use to send or receive messages
};
...
int main(int argc, char *argv[]) {
  ...
  // Declare a thread_args structure for each of the sending and receiving threads
...
void *send_msgs(void *args) {
  ...
  for (int i = 0; i < real_args->num_msgs; i++) {
    // Initialize the mq_msg structure with the appropriate thread id and message id.
...
void *receive_msgs(void *args) {
  ...
  // repeatedly doing the following:
  for (int i = 0; i < real_args->num_msgs; i++) {
    // Receive the message from the queue in the thread_args structure using mq_receive.
    mq_receive(real_args->mq, &r_message);

    // Display the message along with the receiver thread id from the thread_args structure.
